...
contract NFTPoolLockAndRelease is CCIPReceiver, OwnerIsCreator {
    ...
    error InvalidReceiverAddress(); // Used when the receiver address
    ...
    event MessageSent(
        ...
        bytes _text, // The text being sent.
        address feeToken, // the token address used to pay CCIP fees.
        uint256 fees // The fees paid for sending the CCIP message.
    );

    // Event emitted when the token is unLocked .
    event TokenUnLocked(
        uint256 tokenId, // The unique ID of the token.
    ...
    //mapping to keep track of locked tokenId
    ...
    function lockAndSendNFT(
        ...
        //transfer NFT to this addrress to lock the NFT
        nft.transferFrom(msg.sender, address(this), _tokenId);
        //发送跨链消息：需要传入receiver地址和tokenId给链下的ccip组件
    ...
    function _ccipReceive(
        ...
        ReceiveData memory receiveData = abi.decode(
            any2EvmMessage.data,
            (ReceiveData)
        );
        uint256 tokenId = receiveData.tokenId;
        address newOwner = receiveData.newOwner;
        ...
        //Emit an event with the tokenId and new owner
